...
$res = mysqli_query($conexion,$sql)or die('Factura NO realizada con éxito'.mysql_error());
if($res){
	$acumCamposN = "";
	for($i = 1;$i <= $_POST['numCampos']; $i++){
	  if(isset($_POST["id_venta$i"])	){//IF ESTE CAMPO SE ENVIA procedo
		...
		$sql_consul_inven = mysqli_query($conexion, sprintf("SELECT * FROM inventario WHERE inventario.codigo = '%s'",$_POST["fk_inventario$i"]) );
		...
		$UpdateInven = mysqli_query($conexion,$sqlUpdateInven)or 
		...
		if($sqlUpdateInven){
			...
			$InsertRegInv = sprintf("INSERT INTO reg_inventario( fecha_reg_inv, costo_reg_inv, cantidad_reg_inv, pmpvj, tipo, fk_inventario, fk_fact_cv, hora_registro, fecha_registro) VALUES
									...
									date('H:i:s'),
									date('Y/m/d'));
			$resInsertRegInv = mysqli_query($conexion,$InsertRegInv)or
		...
		*/
		...
		$res2 = mysqli_query($conexion,$sql2)or die('Venta NO realizada con éxito'.mysql_error());
		}
