...
function consulTablaAjax(url, tabla, campo, resCampo) {
	var xhttp, resAjax = 0, urlC;
	xhttp = new XMLHttpRequest();

	xhttp.onreadystatechange = function () {
		if (xhttp.readyState == 4 && xhttp.status == 200) {
			if (xhttp.responseText !== 0) {
				resAjax = xhttp.response;//console.log(resAjax);
				cadA = parseFloat(resAjax.substr(-3));//console.log(cadA);
				cadA++;

				document.form1.elements[resCampo].value = cadA;
			}
		}
	};

	urlC = extra + url;
	xhttp.open("GET", urlC + "?tabla=" + tabla + "&columna=" + campo, true);
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp.send();//
}
...
/*para crear el campo de una factura ya q no debe ser autoincremento sino cimpuesto por el serie+numerofactura junto a rif de la empresa del proveedro*/
function codFactProvee(formu, campoA, campoB, campoC, campoRes) {//id_fact_venta

	var value1 = document.forms[formu].elements[campoA].value//serie_fact_venta
	var value2 = document.forms[formu].elements[campoB].value//num_fact_venta
	var value3 = document.forms[formu].elements[campoC].value//fk_cliente
	//alert(value1+value2+value3);
	document.forms[formu].elements[campoRes].value = value1 + value2 + value3

	tabla = campoRes.substring(3);
	valor = document.forms[formu].elements[campoRes].value;
	//funcion para validar que no se repita esta id_fact_venta
	//tabla = tabla	//campoRes = columna//valor = valor//campoA = campo_dinamico//
	validar_repetidoM(tabla, campoRes, valor, campoA)
	//vacio el resto ya vacie campoA que es serie
}
...
function genCodRand(input, sistema) {
	sistema = '';
	rand = parseInt(Math.random() * 1000);
	codigoRand = sistema + rand;
	document.getElementById(input).value = codigoRand;
...
function val_comp_ii_fech(vfecha, zona_dinamica)//(document.form1.fecha_fact_compra.value, 'fecha_fact_compra')
{
	if (document.getElementById(zona_dinamica).value == "") {
		//alert("Debe llenar primero la fecha");
		//document.getElementById(zona_dinamica).value = "";
		//document.getElementById(zona_dinamica).focus();
		//return 0;
	} else {
		objAjax = new XMLHttpRequest()
		objAjax.open("GET", extra + "php/consul_sql/b_cv_inv_fech.php?vfecha=" + vfecha);
		objAjax.onreadystatechange = function () {
			if (objAjax.status == 200 && objAjax.readyState == 4) {
				...
				if (objAjax.responseText == 1) {
					...
					document.getElementById("res_" + zona_dinamica).appendChild(span);
					span.appendChild(document.getElementById("cont_" + zona_dinamica));
				} else if (objAjax.responseText == 2) {
					alert("Disculpe las molestias INVENTARIO INICIAL vacio \nSera redireccionado en cuanto pulce Aceptar");
					document.getElementById(zona_dinamica).value = "";
					setTimeout(window.location = "cargarinventario.php", 10000); //tiempo expresado en milisegundos
					//alerta y focus en campo
					return 0;
				} else {
					alert("La Fecha que ingreso debe ser mayor a la FECHA de INVENTARIO INICIAL: " + objAjax.responseText);
					document.getElementById(zona_dinamica).value = "";
					document.getElementById(zona_dinamica).focus();
					//alerta y focus en campo
					return 0;
