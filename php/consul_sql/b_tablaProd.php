...
{		
		...
		//realizo el sql
		$consulta=mysqli_query($conexion,sprintf("SELECT * FROM fact_compra, compra, inventario WHERE
...
<table class="table_nada table table-bordered table_black">
  <thead id="resCompra">
	...
	if($total_consulta >= 1){
		...
		$consultaPMPVJ = mysqli_query($conexion,sprintf("SELECT * FROM inventario, reg_inventario WHERE
